...
# Helper function for fetching details
def helper(dis):
    try:
        ...
        # Convert strings from lists to actual lists
        med = [ast.literal_eval(item) for item in med] if med else []
        die = [ast.literal_eval(item) for item in die] if die else []
...
def get_predicted_value(patient_symptoms):
    try:
        ...
        # Map prediction index to disease
        disease = diseases_list.get(prediction, "Unknown Disease")
        return disease
    except KeyError as e:
        print(f"KeyError: {e}")
        return "Invalid symptoms detected. Please check input."
    except Exception as e:
...
def predict():
    if request.method == 'POST':
        ...
        print(f"User symptoms: {symptoms}")
        ...
        print(f"Static test predicted: {predicted_disease}")
        
        if predicted_disease == "Prediction failed due to an error.":
            return render_template('index.html', message="Error in prediction. Please try again.")
        
        dis_des, precautions, medications, rec_diet, workout = helper(predicted_disease)
        
        # Print helper output
        print(f"Helper output: desc={dis_des}, precautions={precautions}, medications={medications}, diet={rec_diet}, workout={workout}")
